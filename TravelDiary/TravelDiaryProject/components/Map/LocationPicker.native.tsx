...
const getAddressFromCoords = async (lat: number, lon: number, sea: string): Promise<string> => {
  try {
    ...
    return sea ? (sea + ',' + parts.slice(0, 2).join(', ') || '未知地址') :
      (parts.slice(0, 2).join(', ') || '未知地址');
  } catch (e) {
    console.error(e);
    return '地址获取失败';
  }
...
const LocationPicker: React.FC<Props> = ({ onLocationSelected }) => {
  ...
  const handleMapPress = async (e: MapPressEvent) => {
    ...
    const addr = await getAddressFromCoords(latitude, longitude, searchQuery);
    setAddress(addr);
  ...
  const handleSearch = async () => {
    ...
    const addr = await getAddressFromCoords(coords.lat, coords.lon, searchQuery);
    setAddress(addr);
